...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch(`http://localhost:8080/diaries/user/date?date=${encodeURIComponent(date)}`, {
        ...
        .then(response => {
            if (!response.ok) {
                console.error("일기 데이터 요청 실패, 상태 코드:", response.status);
                throw new Error("일기 데이터를 가져오지 못했습니다.");
            }
        ...
        .then(response => {
            if (!response.ok) {
                throw new Error("감정 데이터를 가져오지 못했습니다.");
            }
            return response.json();
        })
        .then(emotionData => {
...
function displayEmotionResult(emotionData) {
    ...
    if (emotionContainer && emotionData) {
        ...
        emotionData.forEach(item => {
            ...
            emotionHtml += `<h3>${koreanEmotion}: ${percentage}%</h3>`;
        });
...
document.querySelector(".edit-button").addEventListener("click", () => {
    ...
    fetch(`http://localhost:8080/diaries/user/date?date=${encodeURIComponent(date)}`, {
        ...
        .then(data => {
            ...
            window.location.href = `diaryEdit.html?date=${encodeURIComponent(date)}&content=${encodeURIComponent(parsedContent)}`;
        })
        .catch(error => {
            console.error("수정 데이터 로딩 오류:", error.message);
